...
### Livello 1: Pezzo visibile (kit CC0 default)
- Un solo prop per riga, con `source:` esplicito.
- Mai *"crea mobili"* e mai *"modella in Blender la cassa"* se Kenney ce l'ha.
- Sempre:
  - `TASK-XXX`: Dress sedia da Kenney Furniture (`chair.glb`, source=kenney)
  - `TASK-XXX`: Dress tavolo da KayKit Furniture (`table.glb`, source=kaykit)
  - `TASK-XXX`: Dress porta dungeon (`door_leaf.glb`, source=kenney)
  - `TASK-XXX`: (solo se assente dal kit) Import Poly Pizza (`relic.glb`, source=polypizza)
- Evidenza kit: file in `art/exports/` + `screenshots/030-import-*.png` + collider.
- Evidenza polypizza: GLB > 15KB + `blender-*-2-import.png` + finale. Nessun cubo bpy.
...
1. **Generazione Immediata del Manifesto (`art/ASSET_MANIFEST.md`)**:
   ...
   - **Vincolo di Pivot**: Minimo $Z=0$ (base a terra) e centro $X=0, Y=0$.
