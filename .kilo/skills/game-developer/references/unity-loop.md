...
## ProBuilder — Geometria Parametrica In-Editor
Non usare la CLI per modellare. L'agente usa ProBuilder via script C# o manipolazione componenti:
- Creazione stanze, rampe e piattaforme allineate al Geometra: usare `UnityEngine.ProBuilder.ShapeFactory.CreateShape(ShapeType.Cube)` o `CreateShape(ShapeType.Stairs)`.
- Applicazione collider automatica e unione mesh statiche.
- ProBuilder permette di avere una greybox perfetta senza dipendere da software esterni prima della fase di rifinitura artistica.
